...
for i in range(len(atac_names)):
    temp = atac_names[i]
    atac_names[i] = temp[2:-3]
diff_atac_peaks_name = None
for i in range(diff_atac_set.values.shape[0]):
    if diff_atac_peaks_name is None:
        diff_atac_peaks_name = atac_names[diff_atac_set.values[i,:]]
    else:
        diff_atac_peaks_name = np.vstack((diff_atac_peaks_name,atac_names[diff_atac_set.values[i,:]]))
diff_atac_peaks_name_df = pd.DataFrame(diff_atac_peaks_name.T,columns=cell_type_annotation)
diff_atac_peaks_name_df.to_csv(os.path.join(save_path,"diff_atac_peaks_name.csv"))
'''
...
#regression R2 boxplot
gene_clu_names = {0:'axonogenesis',2:'cognition',4:'hematopoietic stem cell differentiation',5:'transport',6:' neuron differentiation',9:'synapse organization',
                  13:'ribonucleoprotein',15:'Ribosome',16:'endomembrane',19:'astrocytes genesis',20:'mRNA processing'}
regression_R2_df = pd.read_csv(save_path+'PLS_prediction_corr_R2_update.csv', header=0, index_col=0)
regression_R2_data = regression_R2_df.values
regression_R2_method_property = regression_R2_data[:,2]
regression_R2_method_property = regression_R2_method_property.astype(np.str)
for i in np.unique(regression_R2_method_property):
    if int(float(i)) == 1:
        regression_R2_method_property[regression_R2_method_property == i] = 'Paired_seq'
    else:
        regression_R2_method_property[regression_R2_method_property == i] = 'scMVP'

regression_R2_clu_property = regression_R2_data[:,1]
regression_R2_clu_property = regression_R2_clu_property.astype(np.str)
for i in np.unique(regression_R2_clu_property):
    regression_R2_clu_property[regression_R2_clu_property == i] = gene_clu_names[int(float(i))]

R2_df = pd.DataFrame({'R2':regression_R2_data[:,0],'gene_clusters':regression_R2_clu_property,'method':regression_R2_method_property},columns=['R2', 'gene_clusters','method'])
...
plt.savefig(save_path+'violinplot_R2', bbox_inches='tight')
plt.savefig(save_path+'violinplot_R2.pdf', bbox_inches='tight')
plt.show()
#plt.figure(figsize=(8, 8))
sns.boxplot(x="gene_clusters",y="R2",data=R2_df,hue="method",palette="hls")
plt.xticks(rotation=90)
plt.savefig(save_path+'boxplot_R2', bbox_inches='tight')
plt.savefig(save_path+'boxplot_R2.pdf', bbox_inches='tight')
plt.show()
R2_df.to_csv(os.path.join(save_path,"boxplot_R2_regression.csv"))
...
peak_clu_TF_exp = peak_clu_TF_exp.T
...
for i in the_geneTF_clu_set.index.values:
    ...
    if np.any(TF_name_column == temp_TF):
    #if index >= 0: # identify the TF family
        temp_value = -np.log(temp_data[2])
        if np.isinf(temp_value):
            temp_value = 690
...
plt.show()
ax = sns.clustermap(gene_clu_spectific_TF_exp_atata+ 0.05*np.random.rand(gene_clu_spectific_TF_exp_atata.shape[0],gene_clu_spectific_TF_exp_atata.shape[1]), metric="correlation",cmap='RdYlBu_r')
plt.show()
rearranged_row = ax.dendrogram_row.reordered_ind
rearranged_col = ax.dendrogram_col.reordered_ind
...
for i in the_geneTF_clu_set.index.values:
    ...
    if np.any(TF_name_column == temp_TF):# identify the TF family
    #if index >= 0:
...
ax = sns.heatmap(gene_clu_spectific_TF_exp_atata,cmap='RdYlBu_r')
plt.show()
ax = sns.clustermap(gene_clu_spectific_TF_exp_atata+ 0.05*np.random.rand(gene_clu_spectific_TF_exp.shape[0],gene_clu_spectific_TF_exp.shape[1]), metric="correlation",cmap='RdYlBu_r')
plt.show()
rearranged_row = ax.dendrogram_row.reordered_ind
rearranged_col = ax.dendrogram_col.reordered_ind
ax = sns.clustermap(gene_clu_spectific_TF_exp_atata + 0.05*np.random.rand(gene_clu_spectific_TF_exp.shape[0],gene_clu_spectific_TF_exp.shape[1]), cmap='RdYlBu_r')
...
for key in gene_clu_spectific_TF_dict.keys():
    ...
    for i in temp_tfs:
        ...
        if sub_clu_TF_enrichment is None:
            ...
            ax.spines['bottom'].set_visible(False)  # outward by 10 points
            plt.xticks([])
            plt.yticks(fontsize=20)
            # 设置坐标标签字体大小
            ax.set_xlabel('')
            ax.set_ylabel('nonneuroCell_genes', fontsize=20)
            ...
            plt.yticks(fontsize=20)
            # 设置坐标标签字体大小
            ax.set_xlabel('pesudotime', fontsize=20)
            ax.set_ylabel('nonneuroCell_peaks', fontsize=20)
            ...
            plt.xticks([])
            plt.yticks([])
            ax.spines['left'].set_visible(False)  # outward by 10 points
            ax.spines['right'].set_visible(False)  # outward by 10 points
            ax.spines['top'].set_visible(False)  # outward by 10 points
            ax.spines['bottom'].set_visible(False)   # outward by 10 points
            # 设置坐标标签字体大小
            ax.set_xlabel('')
            ax.set_ylabel('neuroCell_genes', fontsize=20)
            ...
            # 设置坐标标签字体大小
            ax.set_xlabel('pesudotime', fontsize=20)
            ax.set_ylabel('neuroCell_peaks', fontsize=20)
            #plt.ylabel("neuroCell_peaks")
            #plt.xlabel("pesudotime")

            plt.savefig(save_path + 'neuro.png', bbox_inches='tight')
            plt.savefig(save_path + 'neuro.pdf', bbox_inches='tight')
            ...
            # 设置坐标标签字体大小
            ax.set_xlabel('')
            ax.set_ylabel('gene exp', fontsize=20)
            ...
            # 设置坐标标签字体大小
            ax.set_xlabel('pesudotime',fontsize=20)
            ax.set_ylabel('peak exp',fontsize=20)
            plt.savefig(save_path + 'total_cell.png', bbox_inches='tight')
            plt.savefig(save_path + 'total_cell.pdf', bbox_inches='tight')
...
for i in gene_clu_spectific_TF_exp_dict.keys():
    if np.max(gene_clu_spectific_TF_exp_dict[i]) < 0.2:
        continue
...
geneclu_temp = None
for i in range(len(the_geneclu_0.index.values)): # the specific clu control
    if geneclu_temp is None:
        geneclu_temp =  the_geneclu_0.index.values[i][2:-3]  # the specific clu control
    else:
        geneclu_temp = np.append(geneclu_temp,the_geneclu_0.index.values[i][2:-3]) # the specific clu control
...
for i in (the_atacclu_0.index.values): # the specific clu control
...
feature_filter_rate = len(the_atacclu_0.index.values)/len(the_atacclu_0_unique) # the specific clu control
...
gene_index_cluSpec = np.array(the_geneclu_0.values[:, 0]) # the specific clu control
...
gene_Exp_clu = np.log1p(gene_Exp_clu)
...
cell_time_df = pd.DataFrame({'psudotime':np.round(pusdo_time*50),'gene_exp':gene_Exp_clu,'promoter_exp':atac_PromoterExp_clu,

                            'specific_TF':atac_SepcExp_clu,'general20_TF':atac_generalExp20_clu,'general22_TF':atac_generalExp22_clu})
psudo_label = np.array(cell_time_df['psudotime'].tolist())
...
ax = plt.subplot(511)
sns.boxplot(x = 'psudotime', y = 'gene_exp', data = cell_time_df)
...
plt.xticks([])
plt.subplot(512)
ax = sns.boxplot(x = 'psudotime', y = 'promoter_exp', data = cell_time_df)
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
# 设置坐标标签字体大小
ax.set_xlabel(...,fontsize=15)
...
plt.xticks([])
ax = plt.subplot(513)
sns.boxplot(x = 'psudotime', y = 'specific_TF', data = cell_time_df)
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
# 设置坐标标签字体大小
ax.set_xlabel(...,fontsize=15)
...
plt.xticks([])
plt.subplot(514)
ax = sns.boxplot(x = 'psudotime', y = 'general20_TF', data = cell_time_df)
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
# 设置坐标标签字体大小
ax.set_xlabel(...,fontsize=15)
...
plt.xticks([])
ax = plt.subplot(515)
sns.boxplot(x = 'psudotime', y = 'general22_TF', data = cell_time_df)
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
# 设置坐标标签字体大小
ax.set_xlabel(...,fontsize=15)
...
plt.subplot(231)
c_data = np.array(cell_time_df['psudotime'].tolist())
...
#c_data = (c_data - np.min(c_data))/(max(c_data)-min(c_data))
plt.scatter(x = x_data, y = y_data, c = c_data, cmap='tab20',s = 1)
ax.spines['left'].set_position(('outward', 10))  # outward by 10 points
ax.spines['right'].set_visible(False)  # outward by 10 points
ax.spines['top'].set_visible(False)  # outward by 10 points
ax.spines['bottom'].set_visible(False)  # outward by 10 points
...
# 设置坐标标签字体大小
ax.set_xlabel('')
ax.set_ylabel('promoter',fontsize=15)
ax = plt.subplot(513)
...
# 设置坐标标签字体大小
ax.set_xlabel('')
ax.set_ylabel('Sox11',fontsize=15)
ax = plt.subplot(514)
...
for i in np.unique(cell_clu):
    ...
    psudotime_temp = np.round(psudotime_temp*20)
    ...
    # 设置坐标标签字体大小
    ax.set_xlabel('')
    ax.set_ylabel('promoter', fontsize=18)
    ...
    # 设置坐标标签字体大小
    ax.set_xlabel('')
    ax.set_ylabel('ETV2', fontsize=18)
    ...
    plt.yticks(fontsize=18)
    # 设置坐标标签字体大小
    ax.set_xlabel('pesudotime', fontsize=18)
    ax.set_ylabel('ZNF', fontsize=18)
    plt.show()
